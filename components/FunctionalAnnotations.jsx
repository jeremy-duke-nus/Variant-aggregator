...
const VEPAnnotations = ({searchData}) => {
    ...
    const fetchVepData = async (searchData) => {
        ...
        setLoading(true);
        try {
            const response = await fetch(`https://grch37.rest.ensembl.org/vep/human/hgvs/${searchData.notation}?refseq=1&hgvs=1&content-type=application/json`);
    ...
    useEffect(() => {
        if (VepAnnotations !== null) {
            const flattenedData = VepAnnotations?.flatMap(item => item.transcript_consequences);
    ...
    return (
        <div className="results">
            <h2>Functional Annotations from VEP</h2>
            {isLoading ? <LoadScreen /> 
                        : 
                        tableData !== null ? <AnnotationTable VepAnnotations={tableData} 
                                                /> 
